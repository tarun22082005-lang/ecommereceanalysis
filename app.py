...
def predict():
    ...
    # Pooling instance to have maxlength of 50 tokens
    query_padded = pad_sequences(query_tokenized, padding='post', maxlen=max_len)
    ...
    if query_sentiments[0][0] > 0.5:
        return render_template('index.html',
                               prediction_text=f"Positive Review with probable Rating as: {np.round(query_sentiments[0][0] * 10, 1)}")
    else:
        return render_template('index.html',
                               prediction_text=f"Negative Review with probable Rating as: {np.round(query_sentiments[0][0] * 10, 1)}")
